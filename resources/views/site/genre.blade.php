...
<head>
    ...
    {{-- Title & basic meta (ưu tiên dữ liệu từ $genre) --}}
    @if (!empty($genre))
        <title>{{ $genre->meta_title ?: $genre->name }}</title>

        @if (!empty($genre->meta_description))
            <meta name="description" content="{{ $genre->meta_description }}">
        @elseif (!empty($metaSeo['desc']))
            <meta name="description" content="{{ $metaSeo['desc'] }}">
        @elseif (!empty($settings['description']))
            <meta name="description" content="{{ $settings['description'] }}">
        @endif

        @if (!empty($genre->meta_keywords))
            <meta name="keywords" content="{{ $genre->meta_keywords }}">
        @endif
    @elseif (request()->is('/'))
        ...
        @endif
    @else
        <title>{{ $metaSeo['title'] ?? ($settings['title'] ?? '') }}</title>
    ...
    {{-- Canonical & favicon --}}
    <link rel="canonical" href="{{ route('genre', ['slug' => $genre->slug]) }}">
...
<body>
        ...
        <div class="main-content">
            ...
            <div class="content-left">
                <div id="curr_living_neo" style="display: none;" class="live-lottery">
                    <span class="live-title">Đang
                        tường thuật Xổ số <span id="lottery_living_name">miền Nam</span>
                    </span>
                    <a id="curr_living_btn" href="/xo-so-mien-nam/xsmn-p1.html" rel="nofollow" class="live-btn"
                        title="➜ Xem ngay">➜ Xem ngay</a>
                </div>

                <section class="section">
                    <header class="section-header">
                        <h1>{!! $genre->meta_title !!}</h1>
                    </header>
                    ...
                    <div class="section-content">
                        ...
                        @if ($topBig)
                            @php
                                $img = asset_media($topBig->avatar ?: $topBig->thumbnail ?? '');
                                $href = route('article', [$topBig->slug]);
                                $sum =
                                    $topBig->excerpt ?:
                                    \Illuminate\Support\Str::limit(strip_tags($topBig->content ?? ''), 220);
                            @endphp
                            <article class="article-list">
                                <a href="{{ $href }}" title="{{ $topBig->title }}" class="thumb">
                                    <img alt="{{ $topBig->title }}" class="image" src="{{ $img }}">
                                </a>
                                <header>
                                    <h2 class="article-title first-news">
                                        <a href="{{ $href }}"
                                            title="{{ $topBig->title }}">{{ $topBig->title }}</a>
                                    </h2>
                                    @if ($sum)
                        ...
                        {{-- 4 BÀI NHỎ --}}
                        @foreach ($topSmall4 ?? [] as $a)
                            @php
                                $img = asset_media($a->avatar ?: $a->thumbnail ?? '');
                                $href = route('article', [$a->slug]);
                                $sum =
                                    $a->excerpt ?: \Illuminate\Support\Str::limit(strip_tags($a->content ?? ''), 180);
                            @endphp
                            <article class="article-list">
                                <header>
                                ...
                                <a class="article-image thumb200" href="{{ $href }}"
                                    title="{{ $a->title }}">
                                    <img alt="{{ $a->title }}" class="image lazy" src="{{ $img }}"
                                        data-src="{{ $img }}">
                                </a>
                                @if ($sum)
                                    <div class="article-summary">{{ $sum }}</div>
                ...
                <div class="load-more">
                    <a href="javascript:void(0)" id="loadmore">Xem thêm</a>
                </div>
    ...
    <footer class="footer" id="e_c" p="p-top-10 p-bo-10" h="P90ybwbbDGvSuxYOVzDkHA==">
        <section class="main-content">
            <div class="nav-bottom">
                ...
                <a href="/tin-xo-so-c404-p1.html" class="nav-bottom-item" title="Tin Xổ số">Tin Xổ số</a>
            </div>
            ...
            <div class="footer-content">
                <div class="footer-add">
                    <div class="copyright">
                        ...
                        <p><a href="/tin-xo-so/dieu-khoan-su-dung-va-quy-uoc-su-dung-dich-vu-404-138076.html">Điều
                                khoản thỏa thuận và quy ước sử dụng dịch vụ</a></p>
                        <p><a
                                href="https://xoso.com.vn/tin-xo-so/chinh-sach-bao-mat-va-bao-ve-du-lieu-ca-nhan-tren-website-xoso-com-vn-404-147777.html">Chính
    ...
    <script>
        (function() {
            ...
            const box = document.getElementById('ajaxContentContainer');
            if (!btn || !box) return;

            // Lấy slug từ URL hiện tại: /muc/{slug}
            const slugMatch = window.location.pathname.match(/\/muc\/([^\/?#]+)/);
            ...
            btn.onclick = async function() {
                ...
                try {
                    ...
                    if (!data.status || !data.html.trim()) {
                        btn.textContent = 'Đã hết tin';
                        btn.onclick = null;
                        return;
                    }
